...
export default function DogForm({ addNewDogFunc, closeFormFunc }) {
  ...
  return (
    <div
      ...
      className="absolute w-full h-full bg-gray-700/50 flex items-center justify-center"
    >
      <form
        onClick={(event) => event.stopPropagation()}
        onSubmit={handleSubmit}
        className="bg-green-400 rounded-lg p-5 text-black max-w-md "
      >
        ...
        <button type="submit" className="rounded bg-green-700 text-white p-2">
          Add New Dog
        </button>
        <button
          type="button"
